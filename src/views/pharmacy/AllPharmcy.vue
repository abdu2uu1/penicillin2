...
function getAllPharmcy() {
   ...
   openLoading(true)
   axios.get(`Pharmacies/Get?PageIndex=${filters.value.page}&PageSize=${filters.value.sizePage}`).then((res) => {
...
function getFilters(data) {
   filter.value = false
   openLoading(true)
   axios.get(`Pharmacies/Get?PageIndex=${filters.value.page}&PageSize=${filters.value.sizePage}`, {
      params: {
...
emitter.on('reloadPageAllPharmcy',()=> {
   getAllPharmcy()
})
onMounted(() => {
   openLoading(true)
...
<template>
     <div>
      ...
      <div class="mb-5 mx-4">
         <p>{{ $t("Title_Show_All_pharmcy") }}</p>
      </div>
        <div  class=" mt-6 pa-3 rounded-xl ">
         <AllPharmacyTable nameEmitr="reloadPageAllPharmcy" :items="Data"  :edite="true" />
        </div>
        ...
        <v-dialog v-model="filter" width="1024">
         <FiltersAllPharmcy  @CloseD="filter=false"  @Filters="(e)=>{getFilters(e);closeFilter = true }" />
        </v-dialog>
     </div>
</template>
